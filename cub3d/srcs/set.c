...
/*   Updated: 2023/07/09 15:57:49 by hgeissle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_start_dir(int row, int col, t_game *game)
{
	if (game->map[row][col] == 'E')
		set_pos(1.0, 0.0, &game->p.dir);
	if (game->map[row][col] == 'N')
		set_pos(0.0, -1.0, &game->p.dir);
...
static void	set_start_plane(int row, int col, t_game *game)
{
	...
	if (game->map[row][col] == 'E')
		set_pos(0.0, plane_len, &game->p.plane);
	if (game->map[row][col] == 'N')
		set_pos(plane_len, 0.0, &game->p.plane);
	if (game->map[row][col] == 'W')
		set_pos(0.0, -plane_len, &game->p.plane);
	if (game->map[row][col] == 'S')
		set_pos(-plane_len, 0.0, &game->p.plane);
...
int	set_start_pos(int row, int col, int *start_pos, t_game *game)
{
	if (*start_pos)
		return (0);
	*start_pos = 1;
	set_pos(col + 0.5, row + 0.5, &game->p.pos);
	set_pos(col + 0.5, row + 0.5, &game->p.start_pos);
